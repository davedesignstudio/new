...
## Local PHP + SQLite (no Docker)

From this repo, after PHP 8.3+ and WP-CLI are available:

```bash
bash dphilhower-studio-wp/scripts/run-local-wordpress.sh 8080
```

Open http://127.0.0.1:8080 — Casa Forno and Zarya Table lead the Brands grid. Admin is `studio` / `studio-local` on a fresh local install.
