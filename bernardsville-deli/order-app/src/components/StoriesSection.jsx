      ...
      <div class="container">
        ...
        <div class="stories-grid">
          <For each={STORIES.filter((s) => !s.featured)}>
            {(raw) => {
              const story = () => resolveStory(raw, lang());
              return (
              <article class="story-card fresco-card-bg">
                <button
                  type="button"
                  class="story-card-btn"
                  onClick={() => props.onSelectStory(raw)}
                >
                  ...
                  <div class="story-card-body">
                    ...
                    <h3>{story().title}</h3>
                    <p class="story-excerpt story-blend-text">{story().excerpt}</p>
                    <span class="story-cta">{story().cta ?? (lang() === 'blend' ? 'Read / Читать →' : 'Leggi la storia →')}</span>
                  </div>
                </button>
              </article>
              );
