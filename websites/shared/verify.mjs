#!/usr/bin/env node
/**
 * Pass HP verification — hub + five brands + Crosslink bridges
 */
...
(async () => {
  ...
  check(
    "HUB_CROSSLINK",
    /Crosslink|Hour 3|bridge/i.test(hubText) &&
      (await page.locator("[data-crosslink]").count()) >= 1
  );
  ...
  for (const b of brands) {
    ...
    const brandOk =
      ...
      bridges >= 6;
    check(
      `${b.toUpperCase()}_STAKE+LOGO+PLACE+FITTS+AIL+STREAM+SEAL+EXAMPLE+PATH+WAYS+BRIDGES`,
      brandOk,
      `stream=${stream} seal=${seal} example=${example} bridges=${bridges}`
    );
  }
