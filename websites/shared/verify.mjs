#!/usr/bin/env node
/**
 * Pass HR verification — hub + five brands + Integrate fold + Practice joints
 */
...
(async () => {
  ...
  check(
    "HUB_INTEGRATE",
    /Integrate|Hour 5|CLOSED|Ri stack/i.test(hubText) &&
      (await page.locator("[data-integrate]").count()) >= 1 &&
      (await page.locator("[data-canon-shelves]").count()) >= 1
  );
  check(
    "HUB_PRACTICE_RETAINED",
    (await page.locator("[data-practice-drill] [data-joint][data-stamped='true']").count()) === 6
  );
  ...
  for (const b of brands) {
    ...
    const drillState = await page
      ...
      .getAttribute("data-state");
    const integrate = await page.locator("[data-integrate]").count();
    ...
    const brandOk =
      ...
      shelves >= 6;
    check(
      `${b.toUpperCase()}_STAKE+LOGO+PLACE+FITTS+AIL+STREAM+SEAL+EXAMPLE+PATH+WAYS+BRIDGES+JOINTS+INTEGRATE+SHELVES`,
      brandOk,
  ...
  const holdAil = await page.locator("#hold [data-ail], #hold .ail").count();
