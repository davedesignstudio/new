#!/usr/bin/env node
/**
 * Pass HQ verification — hub + five brands + Practice joint stamps
 */
...
(async () => {
  ...
  check(
    "HUB_PRACTICE",
    /Practice|Hour 4|joint/i.test(hubText) &&
  ...
  for (const b of brands) {
    ...
    const brandOk =
      ...
      drillState === "sealed";
    check(
      `${b.toUpperCase()}_STAKE+LOGO+PLACE+FITTS+AIL+STREAM+SEAL+EXAMPLE+PATH+WAYS+BRIDGES+JOINTS`,
      brandOk,
      `stream=${stream} seal=${seal} stamped=${stamped}/6 drill=${drillState}`
    );
  }
