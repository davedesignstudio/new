#!/usr/bin/env node
/**
 * Pass HO verification — hub + five brands + exampleData pedagogy
 */
...
(async () => {
  ...
  const hubDeepen = await page.locator("body").innerText();
  check("HUB_OK", hubLinks === 5, `cards=${hubLinks}`);
  check("HUB_DEEPEN", /Deepen|exampleData|Hour 2/i.test(hubDeepen));
  ...
  for (const b of brands) {
    ...
    const stream = await page.locator("[data-stream-rail]").getAttribute("data-state");
    const seal = await page.locator("[data-practice-seal]").getAttribute("data-state");
    ...
    const brandOk =
      ...
      twoWays >= 1;
    check(
      `${b.toUpperCase()}_STAKE+LOGO+PLACE+FITTS+AIL+STREAM+SEAL+EXAMPLE+PATH+WAYS`,
      brandOk,
      `stream=${stream} seal=${seal} example=${example} paths=${pathLabel}`
    );
  }
