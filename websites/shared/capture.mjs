...
(async () => {
  ...
  await page.screenshot({ path: path.join(OUT, "hq-hub.png"), fullPage: false });
  ...
  await page.screenshot({
    path: path.join(OUT, "hq-pair-hold.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(OUT, "hq-lumen-joints.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(OUT, "hq-lumen-mobile.png"),
    fullPage: false,
  });
  ...
  const ctx = await browser.newContext({
    viewport: { width: 1280, height: 800 },
    recordVideo: {
      dir: path.join(OUT, "hq-video-tmp"),
  ...
  await ctx.close();
  fs.renameSync(vpath, path.join(OUT, "hq-walkthrough.webm"));
  try {
    fs.rmSync(path.join(OUT, "hq-video-tmp"), { recursive: true });
  } catch {}
