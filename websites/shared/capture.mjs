...
(async () => {
  ...
  await page.screenshot({ path: path.join(OUT, "hr-hub.png"), fullPage: false });
  ...
  await page.screenshot({
    path: path.join(OUT, "hr-pair-hold.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(OUT, "hr-lumen-joints.png"),
    fullPage: false,
  });

  await page.locator("[data-integrate]").scrollIntoViewIfNeeded();
  await page.screenshot({
    path: path.join(OUT, "hr-lumen-integrate.png"),
    fullPage: false,
  });
  await page.locator("[data-canon-shelves]").scrollIntoViewIfNeeded();
  await page.screenshot({
    path: path.join(OUT, "hr-lumen-shelves.png"),
    fullPage: false,
  });
  ...
  await page.screenshot({
    path: path.join(OUT, "hr-lumen-mobile.png"),
    fullPage: false,
  });
  ...
  const ctx = await browser.newContext({
    viewport: { width: 1280, height: 800 },
    recordVideo: {
      dir: path.join(OUT, "hr-video-tmp"),
  ...
  await ctx.close();
  fs.renameSync(vpath, path.join(OUT, "hr-walkthrough.webm"));
  try {
    fs.rmSync(path.join(OUT, "hr-video-tmp"), { recursive: true });
  } catch {}
